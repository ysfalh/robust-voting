...
def draw_curves(l_lists1, l_lists2, l_lists3, l_lists4, l_params, labels=('', '', '', ''), folder='exp1', title='Average correlation', x_name=''):
    ...
    plt.fill_between(list(l_params), list(vals1 - range1), list(np.minimum(1, vals1 + range1)), alpha=0.1, color='orange')
    plt.plot(l_params, vals2, label=labels[1], linestyle='dotted', color='red')
    plt.fill_between(list(l_params), list(vals2 - range2), list(np.minimum(1, vals2 + range2)), alpha=0.1, color='red')
    plt.plot(l_params, vals3, label=labels[2], linestyle='solid', color='green')
    plt.fill_between(list(l_params), list(vals3 - range3), list(np.minimum(1, vals3 + range3)), alpha=0.1, color='green')
    plt.plot(l_params, vals4, label=labels[3], linestyle='dashed', color='blue')
    plt.fill_between(list(l_params), list(vals4 - range4), list(np.minimum(1, vals4 + range4)), alpha=0.1, color='blue')
